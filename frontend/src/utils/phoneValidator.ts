 ...
 * @param phoneNumber - Raw phone number from input
 * @returns Validation result with formatted number
 */
export function validateAndFormatPhone(phoneNumber: string): PhoneValidationResult {
  if (!phoneNumber) {
    return {
      valid: false,
      formatted: '',
 ...
 * @param phoneNumber - Phone number in WhatsApp format (40XXXXXXXXX)
 * @returns Formatted phone number (+40 XXX XXX XXX)
 */
